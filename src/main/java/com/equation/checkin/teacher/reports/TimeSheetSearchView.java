...
public class TimeSheetSearchView extends CustomComponent implements View {
	Statement stm, stmt;
	ResultSet rs, rs1;
	String schoolName;

	public TimeSheetSearchView(Statement stm, Statement stmt, ResultSet rs, ResultSet rs1, String schoolName) {
		this.stm = stm;
		this.rs = rs;
		this.stmt = stmt;
		this.rs1 = rs1;
	...
	public VerticalLayout useECumber() {
		...
		search.addClickListener((e) -> {
			...
			if (!ecNumber.getValue().toString().equals("")) {
				new ECNumberTimeSheet(rs, rs1, stm, stmt).search(value, schoolName);
			} else {
	...
	public void insertTab(TabSheet tabs) {
		...
		for (int x = 0; x < count; x++) {
			if (tabs.getTab(x).getCaption().equals("Time Sheet")) {
				exist = true;
		...
		if (!exist) {
			tabs.addTab(this, "Time Sheet", VaadinIcons.ARCHIVE);
