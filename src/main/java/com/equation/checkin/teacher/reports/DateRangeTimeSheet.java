...
public class DateRangeTimeSheet {
	...
	public void search(String dateFrom, String dateTo, String schoolName) {
		String check = "SELECT * FROM checkin WHERE date BETWEEN '" + dateFrom + "' AND '" + dateTo + "'";
		try {
			...
			if (rs1.next()) {
				...
				String query = "SELECT firstName,surname,time_in,time_out,checkin.ecNumber,checkin.date FROM teachers,checkin WHERE checkin.date BETWEEN '"
						+ dateFrom + "' AND '" + dateTo + "' AND checkin.ecNumber = teachers.ecNumber";
				...
				new TimeSheetPDF().generatePDF(schoolName, "Date Range:" + dateFrom + " To " + dateTo, table, download);
