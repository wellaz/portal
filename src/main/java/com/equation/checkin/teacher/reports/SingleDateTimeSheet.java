...
public class SingleDateTimeSheet {
	ResultSet rs, rs1;
	Statement stm, stmt;

	public SingleDateTimeSheet(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public void search(String date, String schoolName) {
		String check = "SELECT * FROM checkin WHERE date = '" + date + "'";
		try {
			rs1 = stmt.executeQuery(check);
			if (rs1.next()) {
				Table table = new Table(date + " Time Sheet");
				...
				String query = "SELECT firstName,surname,time_in,time_out,checkin.ecNumber FROM teachers,checkin WHERE checkin.date = '"
						+ date + "' AND checkin.ecNumber = teachers.ecNumber";
				...
				while (rs.next()) {
					...
					String errandQuery = "SELECT trip,time_out,time_in FROM errand WHERE ecNumber = '" + ecNumber
							+ "' AND date = '" + date + "'";
				...
				new TimeSheetPDF().generatePDF(schoolName, date, table, download);

				window.setContent(layout);
				UI.getCurrent().addWindow(window);
			} else {
				new Notification("Notification", "No Report Found For This Date :" + date,
