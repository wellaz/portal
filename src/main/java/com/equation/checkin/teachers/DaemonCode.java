...
public class DaemonCode {
	...
	public void process(String ecNumber) {
		...
		String query = "SELECT time_in,time_out FROM checkin WHERE ecNumber = '" + ecNumber + "' AND date = '" + today
				+ "'";
		try {
			String findAnomaly1 = "SELECT time_in,date FROM checkin WHERE ecNumber = '" + ecNumber + "' AND period = '"
					+ CheckinState.PENDING + "' AND date <> '" + today + "' ";
			...
			if (!rs.next()) {
				...
				if (rs1.next()) {
					...
					if (check_out.equals(CheckinState.PENDING)) {
						String query1 = "SELECT trip,time_out,time_in FROM errand WHERE ecNumber = '" + ecNumber
								+ "' AND date = '" + today + "'";
						...
						if (rs.next()) {
							...
							if (err_in.equals(CheckinState.PENDING)) {
								...
								// update errand_in
								String errandUpdate = "UPDATE errand SET time_in = '" + time_in + "',period = '"
										+ errand_period + "' WHERE ecNumber = '" + ecNumber + "' AND trip = '" + trip
										+ "'";
				...
				String anomaly2 = "SELECT trip FROM errand WHERE ecNumber = '" + ecNumber + "' AND period = '"
				...
				if (!rs1.next()) {
					...
					new Notification(
							"<h1>End Of Day Check Out Not Found for day " + last_date
									...
									+ "<br/>Consult the Administrator for assistance<h1/>",
					...
					new Notification(
							"<h1>End Of Day Check Out Not Found for day " + last_date + "<br/>EC Number " + ecNumber
	...
	public void checkOut(String today, String check_in, String time_in, String ecNumber) {
		...
		String updateCheckIn = "UPDATE checkin SET time_out = '" + time_in + "',period = '" + todays_hours
				+ "' WHERE ecNumber = '" + ecNumber + "' AND date = '" + today + "'";
		...
		Notification notif = new Notification("Thanks !!!",
				"<br/>Current Time" + time_in + "<br/> We will meet tomorrow", Notification.Type.WARNING_MESSAGE, true);
		// Customize it
		notif.setDelayMsec(100);
		notif.setPosition(Position.MIDDLE_CENTER);
		notif.setStyleName("auto_checkin_note");
