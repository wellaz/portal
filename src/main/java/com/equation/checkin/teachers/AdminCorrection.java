...
public class AdminCorrection {
	...
	public AdminCorrection(Statement stm, Statement stmt, ResultSet rs, ResultSet rs1) {
		this.rs = rs;
		this.stm = stm;
		this.rs1 = rs1;
		this.stmt = stmt;
	...
	public void correct(String ecNumber, Window myWindow, TextField myTextArea) {
		...
		try {
			...
			if (!rs.next()) {
				...
				String anomaly2 = "SELECT trip,time_out FROM errand WHERE ecNumber = '" + ecNumber + "' AND period = '"
				...
				if (!rs1.next()) {
					...
					submit.addClickListener((e) -> {
						...
						if (!value.equals("")) {
							...
							String updateCheckIn = "UPDATE checkin SET time_out = '" + value + "', period = '" + period
									+ "' WHERE ecNumber = '" + ecNumber + "' AND date = '" + last_date + "'";
					...
					submit.addClickListener((e) -> {
						...
						if (!(value.equals("") || value1.equals(""))) {
							...
							String updateCheckIn = "UPDATE checkin SET time_out = '" + value + "', period = '" + period
									+ "' WHERE ecNumber = '" + ecNumber + "' AND date = '" + last_date + "'";
							String updateErand = "UPDATE errand SET time_in = '" + value1 + "', period = '" + errperiod
									+ "' WHERE ecNumber = '" + ecNumber + "' AND period = '" + CheckinState.PENDING
									+ "' AND trip = '" + trip + "'";
							try {
								stm.executeUpdate(updateCheckIn);
