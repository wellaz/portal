...
public class SingleDateTimeSheet {
	...
	public void search(String date, String schoolName) {
		String check = "SELECT * FROM ancillarycheckin WHERE date = '" + date + "'";
		try {
			...
			if (rs1.next()) {
				...
				String query = "SELECT firstName,surname,time_in,time_out,ancillarycheckin.workerid FROM acillary,ancillarycheckin WHERE ancillarycheckin.date = '"
						+ date + "' AND ancillarycheckin.workerid = acillary.workerID";
				...
				new AncillaryTimeSheetPDF().generatePDF(schoolName, date, table, download);
