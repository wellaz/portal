...
public class WorkerIDTimeSheet {
	ResultSet rs, rs1;
	Statement stm, stmt;

	public WorkerIDTimeSheet(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public void search(String ecNum, String schoolName) {
		String check = "SELECT * FROM ancillarycheckin WHERE workerid = '" + ecNum + "'";
		try {
			...
			if (rs1.next()) {
				...
				String query = "SELECT firstName,surname,time_in,time_out,checkin.date FROM acillary,ancillarycheckin WHERE ancillarycheckin.workerid = '"
						+ ecNum + "' AND ancillarycheckin.workerid = acillary.workerid";
				...
				while (rs.next()) {
					...
					String errandQuery = "SELECT trip,time_out,time_in FROM ancillaryerrand WHERE workerid = '" + ecNum
							+ "' AND date = '" + date + "'";
				...
				new AncillaryTimeSheetPDF().generatePDF(schoolName, "_  EC Number:" + ecNum, table, download);
