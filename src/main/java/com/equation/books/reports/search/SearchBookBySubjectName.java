...
public class SearchBookBySubjectName {

	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchBookBySubjectName(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Panel createContentPanel(String schoolname, String subject) {
		Panel panel = new Panel();

		String query = "SELECT schoolName,title,subject,iSBN,status,quantity FROM books,schools WHERE schools.schoolName = '"
				+ schoolname + "' OR schools.schoolID = '" + schoolname + "' AND books.subject = '" + subject + "'";
		try {
			...
			if (rows > 0) {
				...
				grid.setCaption("Subject: " + subject);
