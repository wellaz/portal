...
public class AssignmentPerformanceTable {
	ResultSet rs, rs1;
	Statement stm, stmt;

	public AssignmentPerformanceTable(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.stm = stm;
		this.stmt = stmt;
		this.rs = rs;
		this.rs1 = rs1;
	...
	public void searchPerformanceReport(String assign_id) {
		String query = "SELECT student_id FROM assignment_marks WHERE assignment_id = '" + assign_id + "'";
		...
		try {
			...
			if (rows > 0) {
				String query1 = "SELECT student_id,mark,total_mark FROM assignment_details,assignment_marks WHERE assignment_details.assinment_id = '"
						+ assign_id + "' ORDER BY mark DESC";
				...
				Window window = new Window("Assignment Results");
				window.center();
				window.setSizeFull();
				...
				new Notification(
						"<h1>It seems that no one has written this assignment!<br/>You may need to remind the kids to write it before it is due!<h1/>",
