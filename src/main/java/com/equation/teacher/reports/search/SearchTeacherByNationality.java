...
public class SearchTeacherByNationality {
	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchTeacherByNationality(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Panel createContetPanel(String schoolID, String nationality) {
		Panel panel = new Panel();

		String selectQuery = "SELECT firstName,surname,gender,dateOfBirth,teachers.ecNumber,nationalID,nationality,dOJS,maritalStatus,mainSubjects,middleName,otherName,dateOfJoiningSchool FROM teachers,teacherestablishment,classes,schools WHERE teachers.nationality  =  '"
				+ nationality + "'  AND schools.schoolID = '" + schoolID + "' OR schools.schoolName = '" + schoolID
				+ "'";
		try {
			...
			if (rows > 0) {
				...
				String selectQuery1 = "SELECT firstName,surname,gender,dateOfBirth,teachers.ecNumber,nationalID,nationality,dOJS,maritalStatus,mainSubjects,middleName,otherName,dateOfJoiningSchool FROM teachers,teacherestablishment,classes,schools WHERE teachers.nationality  =  '"
						+ nationality + "'  AND schools.schoolID = '" + schoolID + "' OR schools.schoolName = '"
						+ schoolID + "'";
				...
				while (rs.next()) {
					...
					String nationalityy = rs.getString(7);
					...
					int period = new CalculateAge().getAge(dOJS);

					grid.addItem(new Object[] { ecNumber, firstName, middleName, surname, otherName, genderr, age,
							nationalID, nationalityy, dOJS, maritalStatus, mainSubjects, dateOfJoiningSchool, period },
							new Integer(i));
				...
				new Notification("Warning", "No record found!", Notification.Type.WARNING_MESSAGE, true)
