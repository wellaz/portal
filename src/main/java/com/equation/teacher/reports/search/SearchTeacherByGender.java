...
public class SearchTeacherByGender {

	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchTeacherByGender(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Panel createContetPanel(String schoolID, String gender) {
		Panel panel = new Panel();

		String selectQuery = "SELECT firstName,surname,gender,dateOfBirth,teachers.ecNumber,nationalID,nationality,dOJS,maritalStatus,mainSubjects,middleName,otherName,dateOfJoiningSchool FROM teachers,teacherestablishment,schools WHERE  teachers.gender = '"
		...
		try {
			...
			if (rows > 0) {
				...
				String selectQuery1 = "SELECT firstName,surname,gender,dateOfBirth,teachers.ecNumber,nationalID,nationality,dOJS,maritalStatus,mainSubjects,middleName,otherName,dateOfJoiningSchool FROM teachers,teacherestablishment,schools WHERE  teachers.gender = '"
						+ gender + "' AND schools.schoolID = '" + schoolID + "' OR schools.schoolName = '" + schoolID
						+ "'";
				...
				while (rs.next()) {
					...
					grid.addItem(
							new Object[] { ecNumber, firstName, middleName, surname, otherName, genderr, age,
									nationalID, nationality, dOJS, maritalStatus, mainSubjects, dateOfJoiningSchool },
							new Integer(i));
