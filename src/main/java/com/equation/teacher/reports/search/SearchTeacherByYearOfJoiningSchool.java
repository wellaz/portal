...
public class SearchTeacherByYearOfJoiningSchool {

	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchTeacherByYearOfJoiningSchool(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Panel createContetPanel(String schoolID, String year) {
		...
		String selectQuery = "SELECT firstName,surname,gender,dateOfBirth,teachers.ecNumber,nationalID,nationality,dOJS,maritalStatus,mainSubjects,middleName,otherName,dateOfJoiningSchool FROM teachers,teacherestablishment,schools WHERE  teachers.dateOfJoiningSchool LIKE '"
		...
		try {
			...
			if (rows > 0) {
				...
				String selectQuery1 = "SELECT firstName,surname,gender,dateOfBirth,teachers.ecNumber,nationalID,nationality,dOJS,maritalStatus,mainSubjects,middleName,otherName,dateOfJoiningSchool FROM teachers,teacherestablishment,schools WHERE  teachers.dateOfJoiningSchool LIKE '"
