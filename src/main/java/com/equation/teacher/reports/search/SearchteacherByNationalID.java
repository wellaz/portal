...
public class SearchteacherByNationalID {

	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table nametable, contactstable, basix, nextofkeen, oleveltable, aleveltable, professiontable;

	public SearchteacherByNationalID(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Panel createContentPanel(String schoolID, String nationalID) {
		Panel panel = new Panel();

		String selectQuery = "SELECT firstName,surname,gender,dateOfBirth,teachers.ecNumber,nationalID,nationality,dOJS,maritalStatus,mainSubjects,middleName,otherName,dateOfJoiningSchool FROM teachers,teacherestablishment,schools WHERE teachers.nationalID = '"
				+ nationalID + "' AND schools.schoolID = '" + schoolID + "' OR schools.schoolName = '" + schoolID
				+ "' ";
		...
		try {
			...
			if (rows > 0) {
				...
				String query1 = "SELECT firstName,surname,gender,dateOfBirth,teachers.ecNumber,nationalID,nationality,dOJS,maritalStatus,mainSubjects,middleName,otherName,dateOfJoiningSchool FROM teachers,teacherestablishment,schools WHERE teachers.nationalID = '"
						+ nationalID + "' AND schools.schoolID = '" + schoolID + "' OR schools.schoolName = '"
						+ schoolID + "' ";
				...
				if (rs.next()) {
					...
					String ecNumberr = rs.getString(5);
					String nationalIDd = rs.getString(6);
					...
					nametable.addItem(new Object[] { ecNumberr, firstName, middleName, surname, otherName, gender, age,
							nationalIDd, nationality, maritalStatus }, 0);
					String subQuery1 = "SELECT gradeOrSubjects,period,cocuricular FROM teachers,teacherestablishment,schools WHERE teachers.ecNumber = '"
							+ ecNumberr + "' AND schools.schoolID = '" + schoolID + "' OR schools.schoolName = '"
							+ schoolID + "'";
