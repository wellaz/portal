...
public class SearchVoucherByAmount {
	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchVoucherByAmount(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
		...
		try {
			...
			if (rows > 0) {
				...
				while (rs.next()) {
					...
					grid.addItem(new Object[] { voucherNumber, paymentMethod, preparedBy, authorisedBy, paidBy, date,
