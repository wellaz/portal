...
public class SearchExpenditureByAmount {
	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchExpenditureByAmount(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
		...
		try {
			...
			if (rows > 0) {
				...
				while (rs.next()) {
					...
					double amountt = rs.getDouble(3);
					String description = rs.getString(4);
					String supplier = rs.getString(5);
					String date = rs.getString(6) + " " + rs.getString(7);
					totalExpenses += amountt;

					grid.addItem(new Object[] { receiptNumberr, voucherNumber, amountt, description, supplier, date },
							new Integer(i));
					i++;
				}
