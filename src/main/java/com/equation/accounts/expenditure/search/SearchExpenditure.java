...
public class SearchExpenditure extends Panel {
	Statement stm, stmt;
	ResultSet rs, rs1;
	private ComboBox<String> searchCriteria;
	String schoolID;

	public SearchExpenditure(Statement stm, Statement stmt, ResultSet rs, ResultSet rs1, String schoolID) {
		this.rs = rs;
		this.stm = stm;
		this.rs1 = rs1;
		this.stmt = stmt;
		...
		searchCriteria.setItems("Amount", "Receipt  / Voucher Number", "Date");
		...
		searchCriteria.addValueChangeListener((e) -> {
			...
			switch (criteria) {
			case "Receipt  / Voucher Number": {
				layout.removeAllComponents();
				TextField depositNumbertxt = new TextField("Receipt / Voucher Number");
				...
				search.addClickListener((ee) -> {
					...
					if (!receiptNumber.equals("")) {
						SearchExpenditureByReceiptNumber byReceiptNumber = new SearchExpenditureByReceiptNumber(this.rs,
			...
			case "Amount": {
				...
				search.addClickListener((ee) -> {
					...
					if (!amoutt.equals("")) {
						...
						SearchExpenditureByAmount byAmount = new SearchExpenditureByAmount(this.rs, this.rs1, this.stm,
								this.stmt);

						UI.getCurrent().addWindow(byAmount.createContentPanel(schoolID, amount));
					} else {
						new Notification("<h1>Empty amount<h1/>", "", Notification.Type.ERROR_MESSAGE, true)
								.show(Page.getCurrent());
					}
				});
				layout.addComponents(searchCriteria, depositNumbertxt, search);
			...
			case "Date": {
				...
				search.addClickListener((ee) -> {
					...
					SearchExpenditureBydate byDate = new SearchExpenditureBydate(this.rs, this.rs1, this.stm,
							this.stmt);
		...
		this.setCaption("Search Expenditure Record");
