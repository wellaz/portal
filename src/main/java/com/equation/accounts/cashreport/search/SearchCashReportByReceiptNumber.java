...
public class SearchCashReportByReceiptNumber {
	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchCashReportByReceiptNumber(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Window createContentPanel(String schoolID, String receiptNumber) {
		...
		String query = "SELECT openingBalance,cashorchequeReceived,firstReceiptNumber,lastReceiptNumber,cashBanked,preparedBy,approvedBy,date FROM cash_report,schools WHERE schools.schoolID = '"
				+ schoolID + "'  AND firstReceiptNumber = '" + receiptNumber + "' OR lastReceiptNumber = '"
				+ receiptNumber + "'";
		try {
