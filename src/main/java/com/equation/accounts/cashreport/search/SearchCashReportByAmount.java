...
public class SearchCashReportByAmount {
	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchCashReportByAmount(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Window createContentPanel(String schoolID, double amount) {
		...
		String query = "SELECT openingBalance,cashorchequeReceived,firstReceiptNumber,lastReceiptNumber,cashBanked,preparedBy,approvedBy,date FROM cash_report,schools WHERE schools.schoolID = '"
				+ schoolID + "'  AND openingBalance = '" + amount + "' OR cashorchequeReceived = '" + amount
				+ "' OR cashBanked = '" + amount + "'";
		try {
