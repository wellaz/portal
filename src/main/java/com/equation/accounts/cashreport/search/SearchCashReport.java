...
public class SearchCashReport extends Panel {
	...
	public SearchCashReport(Statement stm, Statement stmt, ResultSet rs, ResultSet rs1, String schoolID) {
		this.rs = rs;
		this.stm = stm;
		this.rs1 = rs1;
		this.stmt = stmt;
		...
		searchCriteria.setItems("Amount", "Receipt Number", "Date");
		...
		searchCriteria.addValueChangeListener((e) -> {
			...
			case "Receipt Number": {
				...
				search.addClickListener((ee) -> {
					...
					if (!receiptNumber.equals("")) {
						SearchCashReportByReceiptNumber byReceiptNumber = new SearchCashReportByReceiptNumber(this.rs,
			...
			case "Amount": {
				...
				search.addClickListener((ee) -> {
					...
					if (!amoutt.equals("")) {
						...
						SearchCashReportByAmount byAmount = new SearchCashReportByAmount(this.rs, this.rs1, this.stm,
								this.stmt);

						UI.getCurrent().addWindow(byAmount.createContentPanel(schoolID, amount));
					} else {
						new Notification("<h1>Empty amount cannot be submitted<h1/>", "",
			...
			case "Date": {
				...
				search.addClickListener((ee) -> {
					...
					SearchCashReportByDate byDate = new SearchCashReportByDate(this.rs, this.rs1, this.stm, this.stmt);
