...
public class SearchSundryByReceiptNumber {
	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchSundryByReceiptNumber(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Window createContentPanel(String schoolID, String receiptNumber) {
		...
		String query = "SELECT 	receiptInvoiceNumber,withdrawal,deposit,description,date,cashier,receivedfrom FROM sundry_acc,schools WHERE schools.schoolID = '"
				+ schoolID + "'  AND receiptInvoiceNumber = '" + receiptNumber + "'";
		try {
