...
public class SearchSundryAccountByDescription {
	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchSundryAccountByDescription(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Window createContentPanel(String schoolID, String description) {
		...
		String query = "SELECT 	receiptInvoiceNumber,withdrawal,deposit,description,date,cashier,receivedfrom FROM sundry_acc,schools WHERE schools.schoolID = '"
				+ schoolID + "'  AND description LIKE '" + "%" + description + "%" + "'";
		try {
			...
			if (rows > 0) {
				...
				while (rs.next()) {
					...
					String descriptionn = rs.getString(4);
					String date = rs.getString(5);
					String cashier = rs.getString(6);
					String receivedfrom = rs.getString(7);

					grid.addItem(new Object[] { receiptInvoiceNumber, withdrawal, deposit, descriptionn, date, cashier,
				...
				new Notification("<h1>No search Results<h1/>", "", Notification.Type.ERROR_MESSAGE, true)
