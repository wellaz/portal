...
public class SearchBankingRegisterByAmount {

	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchBankingRegisterByAmount(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Window createContentPanel(String schoolID, double amount) {
		...
		try {
			...
			if (rows > 0) {
				...
				while (rs.next()) {
					...
					double amount1 = rs.getDouble(2);
					String messangerid = rs.getString(3);
					...
					grid.addItem(new Object[] { depositNumber, amount1, messangerid, messangername, senderid,
