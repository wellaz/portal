...
public class SearchbankingRegisterBySenderID {

	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchbankingRegisterBySenderID(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Window createContentPanel(String schoolID, String senderID) {
		...
		String query = "SELECT depositNumber,amount,messagerID,messangerIDSign,senderID,senderIDSign,comments,date FROM banking_register,schools WHERE schools.schoolID = '"
				+ schoolID + "'  AND senderID = '" + senderID + "'";
		try {
