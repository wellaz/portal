...
public class OtherAccountsPDF {

	public OtherAccountsPDF() {
	...
	public void generatePDF(Table table, Button download) {
		try {
			String filename = Filename.setFileName() + "other_accounts_report" + ".pdf";
			...
			document.add(new Paragraph("other accounts".toUpperCase(),
			...
			PdfPCell cell = new PdfPCell(new Paragraph("other accounts".toUpperCase() + " Report",
			...
			for (int rows = 0; rows < table.size(); rows++) {
				for (int cols = 0; cols < length; cols++) {
					Item question = table.getContainerDataSource().getItem(rows);
					String a = (String) question.getItemProperty(array[cols].toString()).getValue();
					ptable.addCell(a);
				}
			}
