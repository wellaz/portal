...
public class SearchReceiptBookByReceiptNumber {
	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchReceiptBookByReceiptNumber(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Window createContentPanel(String schoolID, String receiptNumber) {
		...
		String query = "SELECT bookNumber,firstReceiptNumber,finalReceptNumber,date,time,status,userID FROM receiptbooks,schools WHERE schools.schoolID = '"
				+ schoolID + "'  AND firstReceiptNumber = '" + receiptNumber + "' OR finalReceptNumber = '"
				+ receiptNumber + "'";
		try {
			...
			if (rows > 0) {
				...
				while (rs.next()) {
					...
					grid.addItem(
							new Object[] { bookNumber, firstReceiptNumber, finalReceptNumber, date, status, postedBy },
							new Integer(i));
					i++;
				}
