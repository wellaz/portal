...
public class GenerateFeesStructureReport {

	public GenerateFeesStructureReport() {

	}

	public void generatePDF(Table table, String schoolName, String schoolPostal, String schoolPhone, String schoolCell,
			String schoolEmail, Button download) {
		try {
			String filename = Filename.setFileName() + "_feesStructure" + ".pdf";
			Document document = new Document(PageSize.A4, 40, 40, 40, 40);

			String path = System.getProperty("user.home") + File.separatorChar + filename;
			// String path = ApplicationBasePath.basePath() +
			// "/WEB-INF/reports/" + filename;
			...
			document.add(new Paragraph("ministry of primary and secondary education".toUpperCase(),
			...
			document.add(new Paragraph(schoolPostal));
			document.add(new Paragraph(schoolPhone));
			document.add(new Paragraph(schoolCell));
			document.add(new Paragraph(schoolEmail));
			document.add(
					new Paragraph("Generated on " + (DateFormat.getDateInstance(DateFormat.LONG)).format(new Date()),
							FontFactory.getFont(FontFactory.TIMES_ROMAN, 9, Font.ITALIC, BaseColor.BLACK)));
			document.add(new Paragraph("\nSCHOOL FEES STRUCTURE\n\n",
					FontFactory.getFont(FontFactory.TIMES_ROMAN, 30, com.itextpdf.text.Font.BOLD, BaseColor.BLACK)));

			String[] array = table.getColumnHeaders();
			int length = array.length;
			PdfPTable ptable = new PdfPTable(length);

			PdfPCell cell = new PdfPCell(new Paragraph("School Fees Structure.".toUpperCase(),
			...
			document.add(new Paragraph(""));
			// String grantString = ((length - 1) > 1) ? "" + (length - 1) :
			// "None";
