...
public class TeachersOnVacationOrMaternityLeave {
	ResultSet rs, rs1;
	Statement stm, stmt;
	String schoolID;

	public TeachersOnVacationOrMaternityLeave(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt,
			String schoolID) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
	...
	public Table onVacationOrMaternityLeaveTable() {
		Table table = new Table(" TEACHERS ON VACATION/MATERNITY/SICK/MDL LEAVE");
		String query = "SELECT firstName,surname,gender,teachers.ecNumber, fromDate,toDate,replacement,leaveType FROM teachers,teachersonleave,schools WHERE leaveType = '"
				+ TeacherLeaveTypes.MATERNITY_LEAVE + "' OR leaveType = '" + TeacherLeaveTypes.ON_VACATION
				+ "' OR leaveType = '" + TeacherLeaveTypes.SICK_LEAVE + "' OR leaveType = '"
				+ TeacherLeaveTypes.MDL_LEAVE + "' AND schools.schoolID = '" + schoolID + "'";
		...
		table.addContainerProperty("NAME OF REPLACEMENT", String.class, null);
		...
		try {
			...
			if (rows > 0) {
				String query1 = "SELECT firstName,surname,gender,teachers.ecNumber, fromDate,toDate,replacement,leaveType FROM teachers,teachersonleave,schools WHERE leaveType = '"
						+ TeacherLeaveTypes.MATERNITY_LEAVE + "' OR leaveType = '" + TeacherLeaveTypes.ON_VACATION
						+ "' OR leaveType = '" + TeacherLeaveTypes.SICK_LEAVE + "' OR leaveType = '"
						+ TeacherLeaveTypes.MDL_LEAVE + "' AND schools.schoolID = '" + schoolID + "'";
