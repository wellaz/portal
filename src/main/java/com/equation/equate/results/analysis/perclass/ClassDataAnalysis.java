...
public class ClassDataAnalysis extends CustomComponent {
	 ...
	 * 
	 */
	public ClassDataAnalysis(Statement stm, Statement stmt, ResultSet rs, ResultSet rs1) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
		...
		classnames.setItems(new AllClasses(this.stm, this.rs).allClasses());
			...
			if (!(class_name.equals("") || max.equals(""))) {
				if (max.equals("0")) {
					...
					String query3 = "SELECT student_name,math1,math2,matht,eng1,eng2,engt,sho1,sho2,shot,gen1,gen2,gent,ag1,ag2,agt,totalmarks FROM thistermmarks WHERE class_name = '"
							+ class_name + "' AND matht < '" + passMark + "' AND engt < '" + passMark + "' AND shot < '"
							+ passMark + "' AND agt < '" + passMark + "' ORDER BY totalmarks DESC";
					try {
					...
					// }
					// }
					new ClassAnalysisPDF().generatePDF(class_name, table, download, max);
					...
					new GeneratedAnalysisTable(stm, stmt, rs, rs1).createAnalysisWindow(class_name,
	...
	public void insertTab(TabSheet tabs) {
		...
		for (int x = 0; x < count; x++) {
			if (tabs.getTab(x).getCaption().equals("Data Analysis")) {
				exist = true;
		...
		if (!exist) {
			tabs.addTab(this, "Data Analysis", VaadinIcons.UPLOAD);
