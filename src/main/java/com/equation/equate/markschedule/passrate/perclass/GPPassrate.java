 ...
 *
 */
public class GPPassrate {
	ResultSet rs, rs1;
	Statement stm, stmt;

	public GPPassrate(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.stm = stm;
		this.rs1 = rs1;
		this.stmt = stmt;
	...
	public int getGPpassRate(String class_name, String term, String year) {
		...
		int absent = new GetAbsentStudentsForClass(rs, stm).getAbsentNumber(class_name, AllSubjects.GENERAL_PAPER);
		String query = "SELECT COUNT(gent) FROM thistermmarks WHERE class_name = '" + class_name + "' AND gent >= '"
		...
		try {
			...
			if (rs1.next()) {
				...
				String query1 = "SELECT gent FROM thistermmarks WHERE class_name = '" + class_name + "'  AND term = '"
						+ term + "' AND year = '" + year + "' ";
