 ...
 *
 */
public class MathsPassrate {
	ResultSet rs, rs1;
	Statement stm, stmt;

	public MathsPassrate(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.stm = stm;
		this.rs1 = rs1;
		this.stmt = stmt;
	...
	public int getMathspassRate(String class_name, String term, String year) {
		...
		int absent = new GetAbsentStudentsForClass(rs, stm).getAbsentNumber(class_name, AllSubjects.MATHEMATICS);
		String query = "SELECT COUNT(matht) FROM thistermmarks WHERE class_name = '" + class_name + "' AND matht >= '"
		...
		try {
			...
			if (rs1.next()) {
				...
				String query1 = "SELECT matht FROM thistermmarks WHERE class_name = '" + class_name + "'  AND term = '"
						+ term + "' AND year = '" + year + "' ";
