...
public class OveralPassRate {

	ResultSet rs, rs1;
	Statement stm, stmt;

	public OveralPassRate(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.stm = stm;
		this.rs1 = rs1;
		this.stmt = stmt;
	...
	public Table createPassrateTable(String class_name, String term, String year) {
		...
		int absent1 = new GetAbsentStudentsForClass(rs, stm).getAbsentNumber(class_name, AllSubjects.ENGLISH);
		int absent2 = new GetAbsentStudentsForClass(rs, stm).getAbsentNumber(class_name, AllSubjects.SHONA);
		int absent3 = new GetAbsentStudentsForClass(rs, stm).getAbsentNumber(class_name, AllSubjects.MATHEMATICS);
		int absent4 = new GetAbsentStudentsForClass(rs, stm).getAbsentNumber(class_name, AllSubjects.GENERAL_PAPER);
		int absent5 = new GetAbsentStudentsForClass(rs, stm).getAbsentNumber(class_name, AllSubjects.AGRICULTURE);

		String query = "SELECT matht,engt,shot,gent,agt FROM thistermmarks WHERE class_name = '" + class_name
		...
		try {
			...
			int rows = rs1.getRow();
			if (rows > 0) {
				String query1 = "SELECT matht,engt,shot,gent,agt FROM thistermmarks WHERE class_name = '" + class_name
