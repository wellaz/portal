 ...
 *
 */
public class ShonaPassRate {
	ResultSet rs, rs1;
	Statement stm, stmt;

	public ShonaPassRate(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.stm = stm;
		this.rs1 = rs1;
		this.stmt = stmt;
	...
	public int getShonapassRate(String class_name, String term, String year) {
		...
		int absent = new GetAbsentStudentsForClass(rs, stm).getAbsentNumber(class_name, AllSubjects.SHONA);
		String query = "SELECT COUNT(shot) FROM thistermmarks WHERE  shot >= '" + passMark + "' AND class_name = '"
		...
		try {
			...
			if (rs1.next()) {
				...
				String query1 = "SELECT shot FROM thistermmarks WHERE class_name = '" + class_name + "'  AND term = '"
						+ term + "' AND year = '" + year + "' ";
