 ...
 *
 */
public class EnglishGradePassrate {
	ResultSet rs, rs1;
	Statement stm, stmt;

	public EnglishGradePassrate(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.stm = stm;
		this.rs1 = rs1;
		this.stmt = stmt;
	...
	public int getEnglishpassRate(String class_name, String term, String year) {
		...
		int absent = new GetAbsentStudentsForGrade(rs, stm).getAbsentNumber(class_name, AllSubjects.ENGLISH);
		String query = "SELECT COUNT(engt) FROM thistermmarks WHERE class_name LIKE '" + class_name + "%"
				+ "' AND engt >= '" + passMark + "'  AND term = '" + term + "' AND year = '" + year + "'";
		int passrate = 0;
		try {
			rs1 = stmt.executeQuery(query);
			if (rs1.next()) {
				int rows = rs1.getInt(1);
				String query1 = "SELECT engt FROM thistermmarks WHERE class_name LIKE '" + class_name + "%"
