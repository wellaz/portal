...
public class SearchStudentBySportsHouse {
	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchStudentBySportsHouse(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Panel createContentPanel(String schoolID, String housename) {
		Panel panel = new Panel();
		String query = "SELECT studentID,sFirstName,classname,sgender,pSurname,pCell,pEmail,pAdress FROM students,parents,classes,schoolhouses,schools WHERE  schoolhouses.houseName = '"
				+ housename + "' schools.schoolID = '" + schoolID + "' OR schools.schoolName = '" + schoolID + "'";
		try {
			rs1 = stmt.executeQuery(query);
			rs1.last();
			int rows = rs1.getRow(), i = 0;
			if (rows > 0) {
				String query1 = "SELECT studentID,sFirstName,sSurname,classname,sgender,pSurname,pCell,pEmail,pAdress FROM students,parents,classes,schoolhouses,schools WHERE schoolhouses.houseName = '"
						+ housename + "' AND schools.schoolID = '" + schoolID + "' OR schools.schoolName = '" + schoolID
						+ "'";
				...
				grid.setCaption("House Name: " + housename);
