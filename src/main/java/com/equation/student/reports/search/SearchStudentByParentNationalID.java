...
public class SearchStudentByParentNationalID {
	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchStudentByParentNationalID(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Panel createContentPanel(String schoolID, String nationalID) {
		...
		String query = "SELECT 	studentID,sFirstName,sSurname,classname,sgender,pSurname FROM students,parents,classes,schools WHERE parents.	pNationalID = '"
		...
		try {
			...
			if (rows > 0) {
				String query1 = "SELECT studentID,sFirstName,sSurname,classname,sgender,pSurname,pCell,pEmail,pAdress FROM students,parents,classes,schools WHERE parents.	pNationalID = '"
