...
public class SearchStudentBySchoolName {
	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchStudentBySchoolName(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Panel createContentPanel(String schoolID) {
		Panel panel = new Panel();
		String query = "SELECT 	studentID,sFirstName,sSurname,classname,sgender,pSurname FROM students,parents,classes,schools WHERE schools.schoolID = '"
		...
		try {
			...
			if (rows > 0) {
				String query1 = "SELECT studentID,sFirstName,sSurname,classname,sgender,pSurname,pCell,pEmail,pAdress FROM students,parents,classes,schools WHERE schools.schoolID = '"
						+ schoolID + "' OR schools.schoolName = '" + schoolID + "'";
