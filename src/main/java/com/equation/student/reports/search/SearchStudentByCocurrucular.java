...
public class SearchStudentByCocurrucular {
	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchStudentByCocurrucular(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Panel createContentPanel(String schoolID, String cocurrucular) {
		Panel panel = new Panel();
		String query = "SELECT studentID,sFirstName,classname,sgender,pSurname,pCell,pEmail,pAdress FROM students,parents,classes,schoolhouses,schools WHERE students.cocurrucular = '"
				+ cocurrucular + "' AND schools.schoolID = '" + schoolID + "' OR schools.schoolName = '" + schoolID
				+ "'";
		try {
			...
			if (rows > 0) {
				String query1 = "SELECT studentID,sFirstName,sSurname,classname,sgender,pSurname,pCell,pEmail,pAdress FROM students,parents,classes,schoolhouses,schools WHERE students.cocurrucular = '"
						+ cocurrucular + "' AND schools.schoolID = '" + schoolID + "' OR schools.schoolName = '"
						+ schoolID + "'";
				...
				grid.setCaption("Co-currucular Activity: " + cocurrucular.toUpperCase());
