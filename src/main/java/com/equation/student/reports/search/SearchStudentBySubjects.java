...
public class SearchStudentBySubjects {
	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchStudentBySubjects(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Panel createContentPanel(String schoolID, String subject_name) {
		Panel panel = new Panel();
		String query = "SELECT studentID,sFirstName,classname,sgender,pSurname,pCell,pEmail,pAdress FROM students,parents,classes,schools WHERE students.sSurname = '"
				+ subject_name + "' AND schools.schoolID = '" + schoolID + "' OR schools.schoolName = '" + schoolID
				+ "'";
		try {
			...
			if (rows > 0) {
				String query1 = "SELECT studentID,sFirstName,sSurname,classname,sgender,pSurname,pCell,pEmail,pAdress FROM students,parents,classes,schools WHERE  students.sSurname = '"
						+ subject_name + "' AND schools.schoolID = '" + schoolID + "' OR schools.schoolName = '"
						+ schoolID + "'";
				rs = stm.executeQuery(query1);

				grid = new Table("Students details By Subjects");
