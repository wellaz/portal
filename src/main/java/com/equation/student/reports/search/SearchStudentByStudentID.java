...
public class SearchStudentByStudentID {

	ResultSet rs, rs1;
	Statement stm, stmt;
	private Table grid;

	public SearchStudentByStudentID(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Panel createContentPanel(String schoolID, String studentID) {
		...
		try {
			...
			if (rows > 0) {
				...
				while (rs.next()) {
					String studentName = rs.getString(1) + " " + rs.getString(2);
