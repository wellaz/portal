...
public class AssignmentsContinuousAssessment {

	Statement stm, stmt;
	ResultSet rs, rs1;
	String currentClass;

	public AssignmentsContinuousAssessment(Statement stm, Statement stmt, ResultSet rs, ResultSet rs1,
			String currentClass) {
		this.rs = rs;
		this.stm = stm;
		this.stmt = stmt;
	...
	public Window inClassAssessmentWindow(String studentID, String studentName) {
		...
		String query = "SELECT mark,school_subjects.subject,topic,classes.classname,total_mark FROM assignment_details,assignment_marks,school_subjects,classes WHERE student_id = '"
				+ studentID + "' ORDER BY subject ASC ";
		String query1 = "SELECT subject,COUNT(subject),SUM(mark),SUM(total_mark),classname FROM assignment_details,assignment_marks,school_subjects,classes WHERE student_id = '"
		...
		try {
			...
			table2.addContainerProperty("Number Of Assignments", Integer.class, null);
			...
			window.setCaption(studentID + " Assignments Continuout Assessment");
			window.setIcon(VaadinIcons.BRIEFCASE);
			window.setContent(layout);

			new AssignmentsPdf().generatePDF(table, table2, download, studentName, studentID, currentClass);
	...
	public void fetchRepotrt(String studentID, String studentName) {
		String query = "SELECT mark,subject,topic,class_name,total_mark FROM assignmentrecords,assignmentmarkschedule WHERE studentID = '"
				+ studentID + "' ORDER BY subject ASC ";
		try {
			rs = stm.executeQuery(query);
			rs.last();
			...
			if (rows > 0) {
				...
				new Notification("Error",
						"For " + studentID
								+ ", we did not find any perfomance record.<br/>You may need to report this if you are not sure!",
